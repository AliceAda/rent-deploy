  ...
  <div class="page-max">
    <el-card shadow="never">
      ...
      <el-tabs v-model="tab">
        ...
        <el-tab-pane label="经纪人管理" name="agent" />
      ...
      <!-- 经纪人管理 -->
      <div v-else-if="tab === 'agent'">
        <div class="bar">
          <el-button type="primary" @click="openAgentEdit(null)">+ 新增经纪人</el-button>
          <el-button @click="exportAgents">导出</el-button>
        </div>
        <el-table :data="agents" border v-loading="loading">
          <el-table-column prop="name" label="姓名" width="120" />
          <el-table-column prop="phone" label="手机号" width="140" />
          <el-table-column prop="agency" label="所属门店" width="160" />
          <el-table-column label="状态" width="90">
            <template #default="{ row }">
          ...
          <el-table-column label="操作" width="140">
            <template #default="{ row }">
              <el-button size="small" plain @click="openAgentEdit(row)">编辑</el-button>
              <el-button size="small" type="danger" plain @click="deleteAgent(row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-empty v-if="!loading && agents.length === 0" description="暂无经纪人" />
      </div>

      <!-- 菜单管理 -->
      <div v-else-if="tab === 'menu'">
        <div class="bar"><el-button type="primary" @click="$message.info('新增菜单')">+ 新增菜单</el-button></div>
        <el-table :data="menus" border v-loading="loading" row-key="id" :tree-props="{ children: 'children' }" default-expand-all>
          ...
          <el-table-column label="操作" width="160">
            <template #default="{ row }">
              <el-button size="small" plain @click="$message.info('编辑 ' + row.name)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div v-else-if="tab === 'dict'">
        <div class="bar"><el-button type="primary" @click="$message.info('新增字典项')">+ 新增</el-button></div>
      ...
      <!-- 经纪人编辑弹窗 -->
      <el-dialog v-model="agentDialog" :title="agentEditing ? '编辑经纪人' : '新增经纪人'" width="480px">
        <el-form label-width="100px">
          <el-form-item label="姓名"><el-input v-model="agentForm.name" /></el-form-item>
          <el-form-item label="手机号"><el-input v-model="agentForm.phone" /></el-form-item>
          <el-form-item label="所属门店"><el-input v-model="agentForm.agency" /></el-form-item>
          <el-form-item label="佣金比例(%)"><el-input-number v-model="agentForm.commission" :min="0" :max="50" /></el-form-item>
...
const adminEditing = ref<any>(null)
...
async function openAdminEdit(row: any) {
...
const agentForm = reactive({ id: 0, name: '', phone: '', agency: '', status: '正常', commission: 5 })
...
function openAgentEdit(row: any) {
  agentEditing.value = !!row
  if (row) {
    agentForm.id = row.id
    agentForm.name = row.name
    agentForm.phone = row.phone
    agentForm.agency = row.agency
    agentForm.status = row.status
    agentForm.commission = row.commission
  } else {
    agentForm.id = 0
    agentForm.name = ''
    agentForm.phone = ''
...
async function saveAgent() {
  ...
  submitting.value = true
  ElMessage.success(agentEditing.value ? '经纪人已更新' : '经纪人已新增')
  submitting.value = false
...
function exportAgents() {
  const csvContent = agents.value.map(a =>
...
// 系统参数
const params = ref({ auditHour: 24, feeRate: 25, minImg: 3 })
...
async function fetchAll() {
  loading.value = true
  await Promise.all([fetchRoles(), fetchAdmins(), fetchMenus(), fetchDicts(), fetchLogs(), fetchParams(), fetchAgents()])
  loading.value = false
}
