...
-- 13. review 表 - 原表无 status 字段，跳过
-- ALTER TABLE `review` MODIFY COLUMN `status` ...
...
SELECT h.*, COUNT(DISTINCT hi.id) AS image_count, COUNT(DISTINCT uc.id) AS collect_count, AVG(r.rating) AS avg_score, COUNT(DISTINCT r.id) AS review_count
FROM `house` h
...
SELECT h.id, h.title, h.district, h.price, h.status, COUNT(DISTINCT hi.id) AS image_count, COUNT(DISTINCT uc.id) AS collect_count, COUNT(DISTINCT hv.id) AS view_count, AVG(r.rating) AS avg_score, COUNT(DISTINCT r.id) AS review_count
FROM `house` h
LEFT JOIN `house_image` hi ON h.id = hi.house_id
LEFT JOIN `user_collect` uc ON h.id = uc.house_id
...
-- 验证 SQL
-- SELECT COUNT(*) FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'anju_rent' AND REFERENCED_TABLE_NAME IS NOT NULL;
